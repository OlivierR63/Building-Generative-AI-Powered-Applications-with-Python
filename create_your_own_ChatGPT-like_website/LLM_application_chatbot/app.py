...
def get_bot_response(input_text, history):
    ...
    Args:
        ...
        history (list): A list of previous conversation turns (e.g., ["Hi!", "Hello!"]).

    Returns:
        str: The bot's generated response.
    """
    try:
        # Limit the history to 10 exchanges and create conversation history string
        history_string = "\n".join([f"User: {h}" if i % 2 == 0 else f"Bot: {h}" for i, h in enumerate(history[-10:])])
        
        # Tokenize the input text and history
        full_text = f"{history_string}\nUser: {input_text}"
        inputs = tokenizer.encode_plus(full_text, return_tensors="pt", truncation=True, max_length=512)
        ...
        return "Désolé, une erreur est survenue."  # Retourne une réponse par défaut
...
def handle_prompt():
    try:
        ...
        response = get_bot_response(input_text, conversation_histories[session_id])
